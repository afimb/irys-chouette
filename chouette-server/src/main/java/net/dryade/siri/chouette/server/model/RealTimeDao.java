...
public class RealTimeDao extends NamedParameterJdbcDaoSupport
{
	...
	/**
	 * return the SQL formed value for a 'value argument' object
	 * 
	 * @param item object to convert
	 * @return
	 */
	//	protected static String getValue(Object item)
	//	{
	//		// TODO : gerer le cas du booleen si mappe sur un entier /char
	...
	//	      {
	//	        java.sql.Timestamp date = new java.sql.Timestamp(((Date)item).getTime());
	//	        return "'"+date.toString()+"'";
	//	      }
	//		return toSQLString(item.toString());
	//	}
	/**
	 * convert a basic string to an SQL String<p> 
	 * 
	 * append a simple quote to every existing quote<br>
	 * add simple quote at the beginning and end for the string<br>
	 * 
	 * if string is null pointer, return NULL
	 * 
	 * @param inString the string to be converted
	 * @return the sql formed string
	 */
	//	protected static String toSQLString(String inString)
	//	{
	//		int          aFirstQuote;  // position de la première quote
	//		StringBuffer aBuffer;      // buffer de travail
	//		int          anOffset;     // offset du caractère en cours d'analyse dans le buffer
	//
	//		// Code de la méthode ---------------------------------------------------
	//
	...
	//
	//		// test de présence d'une quote
	//		aFirstQuote = inString.indexOf("'");
	//		aBuffer = new StringBuffer(inString);
	//		if (aFirstQuote != -1 )
	//		{
	//			for (anOffset = aFirstQuote ; anOffset < aBuffer.length() ; anOffset++)
	//			{
	//				if (aBuffer.charAt(anOffset) == '\'')
	//				{
	//					aBuffer.insert(anOffset,'\'');
	//					anOffset ++;
	//				}
	//			}
	//		}
